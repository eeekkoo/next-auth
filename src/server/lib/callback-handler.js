...
export default async function callbackHandler (sessionToken, profile, providerAccount, options) {
  ...
  if (providerAccount.type === 'email') {
    ...
    if (userByProviderAccountId) {
      ...
      // 
      // We want to link a users claim profile with their spotify account
      user = providerAccount.id ? await getUserFromUsersByProviderAccountId(providerAccount.id) : null

      if (user) {

      } else if (profile.email ? await getUserByEmail(profile.email) : null) {
      ...
      user = user ?? await createUser(profile)
      await dispatchEvent(events.createUser, user)
